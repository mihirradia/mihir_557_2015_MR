...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	else if (key == 87 && (action == GLFW_PRESS || action == GLFW_REPEAT))
	{
		forward_key++;
		x_coord++; x_coord; cout << " x = "<<x_coord<<"\n";
	}
	else if (key == 83 && (action == GLFW_PRESS || action == GLFW_REPEAT))
	{
		back_key++;
		x_coord--; cout<< " x = " << x_coord << "\n";
	}
	else if (key == 65 && (action == GLFW_PRESS || action == GLFW_REPEAT))
	{
		left_key++;
		y_coord++; cout << " y = " << y_coord << "\n";
	}
	else if (key == 68 && (action == GLFW_PRESS || action == GLFW_REPEAT))
	{
		right_key++;
		y_coord--; cout << " y = " << y_coord << "\n";
	}
...
GLFWwindow* initWindow(void)
{
    ...
    window = glfwCreateWindow(1280, 720, "OpenGL Window for 557", NULL, NULL);
