...
bool GLObjectObj::load_obj(const char* filename, vector<glm::vec3> &vertices, vector<glm::vec3> &normals, vector<glm::vec3> &vtexture, vector<GLuint> &elements)
{
    ...
    while (getline(in, line))
    {
		...
		else if (line.substr(0, 3) == "vt ")
		{
			...
			glm::vec3 vt; s >> vt.x; s >> vt.y; s >> vt.z;
			temp_vtext.push_back(vt);
		}
    ...
    for(int i=0; i<temp_index_triangle.size(); i++)
    {
		...
		glm::vec3 texIdx = temp_index_textures[i];
		...
		vtexture.push_back(temp_vtext[texIdx.x-1]);
		vtexture.push_back(temp_vtext[texIdx.y - 1]);
		vtexture.push_back(temp_vtext[texIdx.z - 1]);
...
void GLObjectObj::initVBO(void)
{
    ...
    for(int i=0; i<_vertices.size() ; i++)
    {
		...
		glm::vec3 vt = _textures[i];
		vertices[(i * 5)] = t.x; vertices[(i * 5) + 1] = t.y; vertices[(i * 5) + 2] = t.z;
		vertices[(i * 5) + 3] = vt.x; vertices[(i * 5) + 4] = vt.y;
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, _num_vertices * 5 * sizeof(GLfloat), vertices, GL_DYNAMIC_DRAW); // Set the size and data of our VBO and set it to STATIC_DRAW
    
    glVertexAttribPointer((GLuint)locPos, 3, GL_FLOAT, GL_FALSE, 5*sizeof(GL_FLOAT), 0); // Set up our vertex attributes pointer
