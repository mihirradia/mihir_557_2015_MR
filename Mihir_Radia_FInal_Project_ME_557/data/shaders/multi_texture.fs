...
uniform sampler2D texture_middle;
...
void main(void)                                                   
{
    ...
    vec4 tex_color_middle = texture(texture_middle, pass_TexCoord);
    ...
    if(texture_blend == 0)
    {
        color = 0.1 * pass_Color + tex_color + tex_color_light +tex_color_middle;
    }
    else if(texture_blend == 1)
    {
        color = tex_color * tex_color_light * tex_color_middle) ;
    }
    else if(texture_blend == 2)
    {
        color = (tex_color_light*0.7)*tex_color * tex_color_middle ;
    }
    else
    {
